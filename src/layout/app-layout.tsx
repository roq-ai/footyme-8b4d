...
import { FiMail, FiUsers, FiMenu, FiUser, FiMessageCircle, FiFile, FiBox, FiCalendar, FiShield } from 'react-icons/fi';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['league-owner'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<LeagueInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/leagues` : null),
    () => getLeagues({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Leagues',
      path: isTenantUser ? `/leagues/view/${data?.[0]?.id}` : '/leagues',
      entity: 'league',
