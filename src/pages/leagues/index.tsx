...
import { getLeagues, deleteLeagueById } from 'apiSdk/leagues';
import { LeagueInterface } from 'interfaces/league';

type ColumnType = ColumnDef<LeagueInterface, unknown>;
...
export function LeagueListPage(props: LeagueListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<LeagueInterface>>(
    () => `/leagues?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: LeagueInterface) => {
    if (hasAccess('league', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/leagues/view/${row.id}`);
    }
  };
...

    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/leagues/view/${record.id}`} passHref legacyBehavior>
            <Button
          ...
          {hasAccess('league', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/leagues/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
          ...
          {hasAccess('league', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="league" />
        </Flex>
        {hasAccess('league', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/leagues/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
